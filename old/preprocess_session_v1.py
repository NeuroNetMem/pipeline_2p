...
output_path = f'/scratch/dspalla/2p_data/{animal}/{session}_int' # output path for temporary preprocessed file
...
#%% cropping
cropped_video_path = output_path+f'/cropped.mmap'
...
if compute_flags['crop']:
    ...
    # crop movie
    movie = fs.crop_movie(movie,cropping_params=cropping_params)
...
mmap_files = glob.glob(str(output_path)+'/cropped_*.mmap')
...
if compute_flags['motion_correct']:
    ...
    mc_input_movie = cropped_video_path
    ...
    new_name = output_path+'/mcorr_'+ motion_corrected_video_file.split('__')[-1]
    fs.rename_file(motion_corrected_video_file,new_name)
...
# MEMORY MAPPING
print('Memory mapping motion corrected file in C order...')
# memory map the file in order 'C'
mmap_files = glob.glob(str(output_path)+'/mcorr_*.mmap')
...
print('Stopping server to clean memory')
cm.stop_server(dview=dview)
